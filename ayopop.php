...
/*      PENGEN GANTI PAYMENT BANK ? REPLACE SADJA 
        ...
        BNI : "mode":"BNIVA" || "paymentChannelCode":"DPVABNI"
*/      
...
class akunNgereff
{
        ...
        public function buySaldoOVO($userId, $authToken, $referralCode, $phoneOvo)
        {
                ...
                $headers[] = "Host: ayopop.com";
                $buySaldoOVO = $this->modules->curl("https://ayopop.com/api/transactions/doRecharge", '{"authToken":"'.$authToken.'","mode":"BNIVA","p_id":1050,"productPrice":"51950","userId":"'.$userId.'","userPhone":"'.$phoneOvo.'","promoCode":"'.$referralCode.'","paymentChannelCode":"DPVABNI"}', false, false, $headers);
                return $buySaldoOVO;
        }
        ...
        public function execute_login($phoneNumber, $password, $referralCode)
        {
                ...
                if(!empty($objLog->userId))
                {
                        ...
                        if(strpos($validateReferral, '"status_code":200'))
                        {
                                ...
                                {
                                        print PHP_EOL."Success Buy ".$objBuy['Product Name']." - Payment : Virtual BNI |Total 53,050| Payment Code : ".$codePay.PHP_EOL;
                                        return false;
                                }else{
...
class akunUtama
{
        ...
        public function checkoutProduct($voucherCode)
        {
                ...
                $headers[] = "Content-Type: application/json; charset=utf-8";
                $headers[] = "Host: ayopop.com";
                $checkoutProduct = $this->modules->curl("https://ayopop.com/api/transactions/doRecharge", '{"authToken":"'.$this->authToken.'","mode":"BNIVA","p_id":701,"productPrice":"55000","userId":"'.$this->userId.'","userPhone":"'.$this->emailUtama.'","promoCode":"'.$voucherCode.'","paymentChannelCode":"DPVABNI"}', false, false, $headers);
        ...
        public function execute_buyProduct($voucherCode)
        {
                ...
                if(strpos($validateVoucher, '"status_code":200'))
                {
                        ...
                        {
                                print PHP_EOL."Success Buy : ".$objCheckout['Product Name']." - Payment : Virtual BNI |Total 35,650| Payment Code : ".$objCheckout['paymentCode'];
                                return false;
                        }else{
